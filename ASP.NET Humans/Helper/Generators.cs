...
namespace ASP.NET_Humans
{
    ...
    {
        static object locker_save = new object();
        static object locker_predict = new object();
        ...
        public static List<Worker> GenerateWorkersWithPhoto(int count, string name) //TODO if Prediction old chance > 40 set Age >35
        {
            List<Worker> workers = new List<Worker>();
            var id_folder_path = @$"wwwroot\Images\{name}";
            ...
            foreach (Worker worker in workers)
            {
                ...
                while (!done)
                {
                    ...
                    if (photoName != null)
                    {
                        File.Move(@$"wwwroot\Images\Checked\{photoName}", @$"wwwroot\Images\{name}\{worker.Sex.ToString().ToLower()}_{worker.Name}.jpg");
                        done = true;
                    }
                    else
                    {
